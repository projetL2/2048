...
void do_move (grid g, dir d) {
    ...
    case DOWN:
        for (int colonne=0; colonne<GRID_SIDE; ++colonne) { // on parcourt de gauche à droite
		  ...
		  if (get_tile(colonne,ligne+1) == 0) {
		    set_tile(colonne,ligne+1,get_tile(colonne,ligne));
		    set_tile(colonne,ligne,0);
                        ligne+=2;
                    }
		  else if (get_tile(colonne,ligne+1) == get_tile(colonne,ligne)) {
		    set_tile(colonne,ligne,0);
		    set_tile(colonne,ligne+1,get_tile(colonne,ligne+1)* 2);
		    g->score += get_tile(colonne,ligne+1);
                    }
                }
            }
        }
    ...
    case UP:
		...
		if (get_tile(colonne,ligne-1) == 0) {
		  ...
		  set_tile(colonne,ligne,0);
		  set_tile(colonne,ligne,get_tile(colonne,ligne)*2);
		  g->score += get_tile(colonne,ligne-1);
                    }
                }
            }
...
// peut être avec un tableau de int avec le 1er chiffre la colonne et  2 eme chiffre la colonne
    ...
    while(get_tile(colonneRand,ligneRand)!=0) { // si la case est occupé on refait des random jusqu'a en trouver une vide.
